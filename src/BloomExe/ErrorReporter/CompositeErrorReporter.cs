...
namespace Bloom.ErrorReporter
{
	...
	class CompositeErrorReporter : IBloomErrorReporter
	{
		...
		public void NotifyUserOfProblemCustom(string message, Exception exception, IRepeatNoticePolicy policy,
			bool shouldHideReportButton,
			string extraButtonLabel, Action<string, Exception> onExtraButtonClicked)
		{
			foreach (var reporter in Reporters)
			{
				reporter.NotifyUserOfProblemCustom(message, exception, policy, shouldHideReportButton, extraButtonLabel, onExtraButtonClicked);
			}
		}
